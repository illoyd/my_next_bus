class StopDataForPredictorDatasetQuery < SimpleDelegator
  ...
  def initialize(base = StopRequest, window = 8.weeks.ago)
    __setobj__ base.where('created_at >= ? and stop_sid is not null and day_of_week is not null and minute_of_day is not null', window).pluck(:day_of_week, :minute_of_day, :stop_sid)
  end
